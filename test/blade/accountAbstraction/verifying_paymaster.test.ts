...
import { SimpleAccount, EntryPoint, VerifyingPaymaster, VerifyingPaymaster__factory } from "../../../typechain-types";
import { createAccount, createAccountOwner, createAddress, deployEntryPoint, simulationResultCatch } from "./testutils";
...
describe("EntryPoint with VerifyingPaymaster", function () {
  ...
  describe("#validatePaymasterUserOp", () => {
    it("should reject on no signature", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(userOp)).to.be.rejectedWith(
    ...
    it("should reject on invalid signature", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(userOp)).to.be.rejectedWith("ECDSA: invalid signature");
    });
    ...
    describe("with wrong signature", () => {
      ...
      it("handleOp revert on signature failure in handleOps", async () => {
        await expect(entryPoint.estimateGas.handleOps([wrongSigUserOp], beneficiaryAddress)).to.rejectedWith(
