...
describe("EntryPoint", function () {
  ...
  before(async function () {
    ...
    ({ proxy: account, accountFactory: simpleAccountFactory } = await createAccount(
      ethersSigner,
  ...
  describe("Stake Management", () => {
    ...
    describe("without stake", () => {
      it("should fail to stake without value", async () => {
        await expect(entryPoint.addStake(2)).to.rejectedWith("no stake specified");
      });
      it("should fail to stake without delay", async () => {
        await expect(entryPoint.addStake(0, { value: ONE_ETH })).to.rejectedWith("must specify unstake delay");
      });
      it("should fail to unlock", async () => {
        await expect(entryPoint.unlockStake()).to.rejectedWith("not staked");
      });
    });
    describe("with stake of 2 eth", () => {
      ...
      it("should fail to withdraw before unlock", async () => {
        await expect(entryPoint.withdrawStake(AddressZero)).to.rejectedWith("must call unlockStake() first");
      });
      describe("with unlocked stake", () => {
        ...
        it("should report unstake state", async () => {
          const withdrawTime1 =
            (await ethers.provider.getBlock("latest").then((block) => block.timestamp)) + globalUnstakeDelaySec;
          const { stake, staked, unstakeDelaySec, withdrawTime } = await entryPoint.getDepositInfo(addr);
          expect({ stake, staked, unstakeDelaySec, withdrawTime }).to.eql({
            stake: parseEther("3"),
            staked: false,
        ...
        it("should fail to withdraw before unlock timeout", async () => {
          await expect(entryPoint.withdrawStake(AddressZero)).to.rejectedWith("Stake withdrawal is not due");
        });
        it("should fail to unlock again", async () => {
          await expect(entryPoint.unlockStake()).to.rejectedWith("already unstaking");
        });
  ...
  describe("#simulateValidation", () => {
    ...
    it("should fail if validateUserOp fails", async () => {
      ...
      //console.log(await entryPoint.callStatic.simulateValidation(op).catch(simulationResultCatch))
      // await expect(entryPoint.callStatic.simulateValidation(op)).to
      //   .rejectedWith('AA25 invalid account nonce')
      await expect(entryPoint.callStatic.simulateValidation(op)).to.rejectedWith("AA25 invalid account nonce");
    });
    ...
    it("should revert if wallet not deployed (and no initcode)", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(op)).to.be.rejectedWith("AA20 account not deployed");
    });
    ...
    it("should revert on oog if not enough verificationGas", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(op)).to.rejectedWith("AA23 reverted (or OOG)");
    });
    ...
    it("should prevent overflows: fail if any numeric value is more than 120 bits", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(op)).to.rejectedWith("gas values overflow");
    });
    ...
    it("should fail creation for wrong sender", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(op1)).to.rejectedWith("AA14 initCode must return sender");
    });
    ...
    it("should report failure on insufficient verificationGas (OOG) for creation", async () => {
      ...
      await expect(entryPoint.callStatic.simulateValidation(op0, { gasLimit: 1e6 })).to.rejectedWith(
        "ValidationResult"
      );
      ...
      await expect(entryPoint.callStatic.simulateValidation(op1, { gasLimit: 1e6 })).to.rejectedWith(
    ...
    it("should not call initCode from entrypoint", async () => {
      ...
      const op1 = await fillAndSign(
        {
          initCode: hexConcat([account.address, account.interface.encodeFunctionData("execute", [sender, 0, "0x"])]),
          sender,
        },
        accountOwner,
        entryPoint
  ...
  describe("#simulateHandleOp", () => {
    it("should simulate execution", async () => {
      ...
      const ret = await entryPoint.callStatic
        .simulateHandleOp(userOp, counter.address, counter.interface.encodeFunctionData("counters", [account.address]))
        .catch((e) => e.errorArgs);
  ...
  describe("flickering account validation", () => {
    ...
    it("should limit revert reason length before emitting it", async () => {
      ...
      const testRevertAccount = await new TestRevertAccount__factory(ethersSigner).deploy(entryPoint.address, {
        value: parseEther("1"),
      });
      ...
      await expect(entryPoint.simulateValidation(badOp, { gasLimit: 3e5 })).to.rejectedWith("ValidationResult");
  ...
  describe("2d nonces", () => {
    ...
    it("should fail nonce with new key and seq!=0", async () => {
      ...
      await expect(entryPoint.callStatic.handleOps([op], beneficiaryAddress)).to.rejectedWith(
  ...
  describe("without paymaster (account pays in eth)", () => {
    describe("#handleOps", () => {
      ...
      it("should revert on signature failure", async () => {
        ...
        const beneficiaryAddress = createAddress();
        await expect(entryPoint.estimateGas.handleOps([op], beneficiaryAddress)).to.rejectedWith(
      ...
      it("account should pay for tx", async function () {
        ...
        console.log(
          "  == est gas=",
          await entryPoint.estimateGas.handleOps([op], beneficiaryAddress, { maxFeePerGas: 1e9 }).then(tostr)
        );
      ...
      it("should report failure on insufficient verificationGas after creation", async () => {
        ...
        // must succeed with enough verification gas
        await expect(entryPoint.callStatic.simulateValidation(op0)).to.rejectedWith("ValidationResult");
        ...
        await expect(entryPoint.callStatic.simulateValidation(op1)).to.rejectedWith("AA23 reverted (or OOG)");
      });
    });
    ...
    describe("create account", () => {
      ...
      it("should succeed to create account after prefund", async () => {
        ...
        await expect(await ethers.provider.getCode(preAddr).then((x) => x.length)).to.equal(
          2,
          "account exists before creation"
        );
    ...
    describe("aggregation tests", () => {
      ...
      before(async () => {
        ...
        aggAccount = await new TestAggregatedAccount__factory(ethersSigner).deploy(
          entryPoint.address,
          aggregator.address
        );
        aggAccount2 = await new TestAggregatedAccount__factory(ethersSigner).deploy(
          entryPoint.address,
          aggregator.address
        );
      ...
      it("should fail to execute aggregated account without an aggregator", async () => {
        ...
        await expect(entryPoint.handleOps([userOp], beneficiaryAddress)).to.rejectedWith("AA24 signature error");
      });
      ...
      it("should run with multiple aggregators (and non-aggregated-accounts)", async () => {
        ...
        const aggAccount3 = await new TestAggregatedAccount__factory(ethersSigner).deploy(
          entryPoint.address,
          aggregator3.address
        );
      ...
      describe("execution ordering", () => {
        ...
        context("create account", () => {
          ...
          before(async () => {
            const factory = await new TestAggregatedAccountFactory__factory(ethersSigner).deploy(
              entryPoint.address,
              aggregator.address
            );
    ...
    describe("with paymaster (account with no eth)", () => {
      ...
      it("simulateValidation should return paymaster stake and delay", async () => {
        ...
        const { stake: simRetStake, unstakeDelaySec: simRetDelay } = paymasterInfo;
    ...
    describe("Validation time-range", () => {
      ...
      describe("validatePaymasterUserOp with deadline", function () {
        ...
        describe("time-range overlap of paymaster and account should intersect", () => {
          ...
          it("handleOps should revert on expired paymaster request", async () => {
            ...
            await expect(entryPoint.handleOps([userOp], beneficiary)).to.rejectedWith(
      ...
      describe("handleOps should abort on time-range", () => {
        it("should revert on expired account", async () => {
          ...
          console.log(await entryPoint.handleOps([userOp], beneficiary).catch((e) => e.reason));
          await expect(entryPoint.handleOps([userOp], beneficiary)).to.be.rejectedWith("AA22 expired or not due");
        });
        ...
        it("should revert on date owner", async () => {
          ...
          await expect(entryPoint.handleOps([userOp], beneficiary)).to.rejectedWith("AA22 expired or not due");
        });
      });
    });
  });
