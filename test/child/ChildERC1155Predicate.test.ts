...
describe("ChildERC1155Predicate", () => {
  ...
  before(async () => {
    ...
    const _proxy = await TransparentUpgradeableProxy.deploy(childERC1155Predicate.address, _proxyAdmin, "0x");
    await _proxy.deployed();
    childERC1155Predicate = ChildERC1155Predicate.attach(_proxy.address);
