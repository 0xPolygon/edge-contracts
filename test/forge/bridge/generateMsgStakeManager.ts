...
const DOMAIN = ethers.utils.arrayify(ethers.utils.solidityKeccak256(["string"], ["STAKE_MANAGER"]));
...
async function generateRegistrationSignature(signerAddress: string, contractAddress: string) {
  ...
  const { signature } = mcl.signStakeManagerMessage(DOMAIN, CHAIN_ID, signerAddress, contractAddress, secret);
