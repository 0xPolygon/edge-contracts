...
// NOTE: Renamed Withdrawal to Withdrawal_Struct because of a conflict with the event of the same name when flattened.
...
struct WithdrawalQueue {
    uint256 head;
    uint256 tail;
    mapping(uint256 => Withdrawal_Struct) withdrawals;
}
...
library WithdrawalQueueLib {
    ...
    function append(WithdrawalQueue storage self, uint256 amount, uint256 epoch) internal {
        ...
        if (tail == head) {
            self.withdrawals[tail] = Withdrawal_Struct(amount, epoch);
            self.tail++;
            return;
        }
    ...
    function withdrawable(
        ...
        for (newHead = self.head; newHead < self.tail; newHead++) {
            Withdrawal_Struct memory withdrawal = self.withdrawals[newHead];
    ...
    function pending(WithdrawalQueue storage self, uint256 currentEpoch) internal view returns (uint256 amount) {
        ...
        for (uint256 i = tail - 1; i >= self.head; i--) {
            Withdrawal_Struct memory withdrawal = self.withdrawals[i];
